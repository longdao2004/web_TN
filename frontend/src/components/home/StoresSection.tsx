    ...
    <Section bgClass="bg-white">
      <PageContainer>
        <div className="flex flex-col items-center justify-between gap-4 sm:flex-row mb-10">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Cửa hàng tiêu biểu</h2>
            <p className="mt-2 text-sm text-gray-500">Các nhà cung cấp uy tín, đạt chứng nhận chất lượng</p>
          </div>
          <Link href="/stores">
          ...
          {stores.map((store) => (
            ...
            >
              ...
              <div className="relative flex flex-1 flex-col p-5 pt-12 text-center">
                ...
                <h3 className="font-bold text-gray-900 group-hover:text-[var(--color-primary)] transition-colors line-clamp-1">{store.name}</h3>
                
                {/* Badges */}
                {store.badges.length > 0 && (
                  <div className="mt-2 flex items-center justify-center gap-1">
                    {store.badges.map((badge, idx) => (
                      <Badge key={idx} variant="secondary" size="sm">{badge}</Badge>
                    ))}
                  </div>
                )}
