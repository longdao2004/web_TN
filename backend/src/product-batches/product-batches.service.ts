...
export class ProductBatchesService {
  ...
  async create(userId: string, createProductBatchDto: CreateProductBatchDto) {
    ...
    if (product.store.ownerId !== userId) {
      throw new ForbiddenException('Bạn không có quyền tạo lô hàng cho sản phẩm này');
    }
   ...
   */
  async update(userId: string, batchId: string, updateProductBatchDto: UpdateProductBatchDto) {
  ...
  async remove(userId: string, userRole: string, batchId: string) {
    ...
    const orders = await this.prisma.orderItem.findFirst({ where: { productBatchId: batchId } });
    if (orders) {
      throw new ForbiddenException('Không thể xóa lô hàng đã có người đặt mua. Hãy cập nhật số lượng về 0 thay vì xóa.');
    }
